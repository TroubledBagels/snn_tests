...
if __name__ == "__main__":
    n_c = 38
    max_classifiers = 20
    ensemble = ae.AdaEnsembleAudio(out_c=n_c, max_classifiers=max_classifiers)

    ds = fd.MFCCDataset()
    print(ds[0][0].shape)

    train_ds, test_ds = torch.utils.data.random_split(ds, [int(0.8*len(ds)), len(ds)-int(0.8*len(ds))])
    trained_ensemble, test_acc = ae.train_ensemble(ensemble, n_c, train_ds, test_ds, num_classifiers=max_classifiers, epochs_per_classifier=3)
