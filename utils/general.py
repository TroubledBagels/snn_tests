...
def train(model, train_dl, test_dl, device, loss_fn=nn.CrossEntropyLoss(), lr=1e-4, epochs=10, weight_decay=0, save_name=None, n_c=75):
    ...
    f1_df = pd.DataFrame(columns=['Epoch'] + [f'Class_{i}_F1' for i in range(n_c)], index=None)
    rec_df = pd.DataFrame(columns=['Epoch'] + [f'Class_{i}_Rec' for i in range(n_c)], index=None)
    prec_df = pd.DataFrame(columns=['Epoch'] + [f'Class_{i}_Prec' for i in range(n_c)], index=None)
    ...
    for epoch in range(epochs):
        ...
        for class_idx in range(len(test_f1)):
            ...
            rec_row[f'Class_{class_idx}_Rec'] = test_rec[class_idx]
            prec_row[f'Class_{class_idx}_Prec'] = test_prec[class_idx]
...
def train_reg(model, train_dl, test_dl, device, loss_fn=nn.CrossEntropyLoss(), lr=1e-4, epochs=10, weight_decay=0,
          save_name=None, n_c=75):
    print("Training model...")
    optimiser = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)
    lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimiser)
    print(f"Using optimiser: {optimiser}")
    ...
    for epoch in range(epochs):
        ...
        for i, (inputs, labels) in enumerate(pbar):
            ...
            pbar.set_description(f"Epoch {epoch + 1}, Batch {i + 1}, Loss: {avg_loss:.4f}, Train Acc: {avg_acc:.4f}")
        pbar.close()
        test_acc, test_loss, test_f1, test_rec, test_prec = test_reg(model, test_dl, device, loss_fn)
        test_acc_rec.append(test_acc)
        test_loss_rec.append(np.mean(test_loss))
        test_f1_rec.append(test_f1)
        lr_scheduler.step(np.mean(test_loss))
...
def test_reg(model, test_dl, device, loss_fn):
    model.to(device)
    model.eval()
    ...
    with torch.no_grad():
        for i, (inputs, labels) in enumerate(pbar):
            ...
            correct += (preds.squeeze() == labels).sum().item()
            total += labels.size(0)
            top3 = outs.topk(3, dim=1).indices
            top5 = outs.topk(5, dim=1).indices
            top10 = outs.topk(10, dim=1).indices
            for j in range(labels.size(0)):
                if labels[j] in top10[j]:
                    top10_correct += 1
                if labels[j] in top5[j]:
